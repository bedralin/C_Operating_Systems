...
/*
	...
	pid = fork();
        ...
        wait(&status);
        printf("testing \n");
        printf("child[0]->args[0]: %s\n",child[0].args[0]);
        if (execvp(child[0].args[0], &child[0].args[0])) {
            puts(strerror(errno));
...
*/
	...
	//wait(&status);
	j = j + 1;
	printf("array count is %d and i is %d\n",array_count,i);
        //pipe(*pfd[i]);                
        if (pid == -1) {
            perror("fork");
            exit(1);
            ...
            printf("Child (child[i].args[0] = %s) speaking...\n", child[i].args[0]);
            printf("Child (child[i].args[0] = %s) speaking...\n", child[1].args[0]);
            wait(&status);
	    if (i == 1) {
		dup2(child[i].fd[0],0);
		close(child[i].fd[0]);  // child does not need this end of pipe
		printf("i is %d\n",i);
	    }
	    if (i == 0) {
		dup2(child[i].fd[1],1);
